...
void Simulazione(){
  ...
  Read_dat_simulation("Dati_Simulazione.dat");// legge i dati e li assegna alle variabili
  ...
  for(Int_t count_z=0; count_z<=1; count_z++){ // Estrai=1 fz->gauss, Estrai=0 fz->scelta 
    ...
    for(Int_t i=0;i<nevent;i++){
      ...
      pt.Point(rms_xy,rms_z,EstraiVert); // genera il vertice con la distribuzione assegnata 

      vtx.x0=pt.GetX(); //coordinate vertice, 
      vtx.y0=pt.GetY(); 
      vtx.z0=pt.GetZ();

      pl.Molteplicita(); // Kinem_File
      mult = pl.GetM();  
      pl.Pseudorapidita(mult);
      vtx.m = mult;
      ...
      for(Int_t j=0; j<mult; j++){
        ...
        if(z1_smear>=-lung_beam/2&&z1_smear<=lung_beam/2){
          ...
          // R=Raggio[2];
            ...
            Label++; // alla fine del loop sulla molteplicità cambia il Label: partielle probenienti da molt diverse avranno molt diverse
        }
